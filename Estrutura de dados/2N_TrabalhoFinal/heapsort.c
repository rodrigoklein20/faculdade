...
void criaHeap(int *vet, int i,int f){ //Função que constrói a heap. Recebe o vetor, o início e o final dele, respectivamente;
    ...
    while(j <= f){                    //Enquanto j for menor ou igual ao final do vetor continua;
        ...
        if(aux<vet[j]){               //Teste se o pai(aux) é menor que o seu filho, se sim:
            ...
            j = f+1;                  //j recebe o fim do vetor mais 1, encerrando a execução do while;
        }
    }
...
int main () {
    ...
    inicio = clock();                          //Registra em clock_t inicio o momento de execução desta chamada
    heapSort(vetor, TAMANHO);                  //Organiza o vetor recebido como parâmetro com o método de ordenação heapSort. Função declarada e comentada acima.
    fim = clock();                             //Registra em clock_t fim o momento de execução desta chamada
    diferenca = fim - inicio;                  //Calcula o valor da subtração de "fim" com "inicio" para registrarmos a diferença de tempo entre as
                                               //duas chamadas, diferença causada nesse caso pela função heapSort que está entre elas.
    printf("\nTempo em ms: %10.4f\n", diferenca/(CLOCKS_PER_SEC/1000));  //Mostra o tempo tomado pela função de ordenação em milissegundos
    escreve_vetor(vetor, "VETOR ORDENADO");    //Escreve novamente o vetor, porém neste momento já ordenado
}
